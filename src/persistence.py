...
def _target_to_dict(target: LinkedTarget) -> dict:
    return {
        "path": target.path,
        "target_type": target.target_type,
        "state": target.state,
        "last_generated_at": target.last_generated_at,
        "last_error": target.last_error,
    }


def _record_to_dict(record: SourceLinkRecord) -> dict:
    return {
        "schema_version": SCHEMA_VERSION,
        "source": {
            "path": record.source_path,
            "last_seen_mtime": record.source_last_seen_mtime,
            "last_seen_size": record.source_last_seen_size,
        },
        "targets": [_target_to_dict(target) for target in record.targets],
        "updated_at": record.updated_at,
    }


def _coerce_target(raw: dict) -> LinkedTarget | None:
    path = raw.get("path")
...
def _coerce_record(source_path: str, data: dict) -> SourceLinkRecord:
    source_abs = _normalize_abs(source_path)
    ...
    # Future-proof behavior: if the persisted source path does not match the current
    # source path, keep current source path but retain target records.
    return SourceLinkRecord(
...
def load_source_links(source_path: str) -> SourceLinkRecord:
    ...
    record = _coerce_record(source_abs, payload)
